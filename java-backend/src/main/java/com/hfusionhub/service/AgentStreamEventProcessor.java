...
public class AgentStreamEventProcessor {
    ...
    public AgentStreamEventProcessor(@Lazy AgentTaskService agentTaskService,
    ...
    public Optional<JsonNode> handleLine(String rawChunk, Long runId) {
        ...
        try {
            ...
            switch (eventType) {
                ...
                case "run_started" -> { /* no persistence needed — run created by startRun() */ }
    ...
    private void handleStepCompleted(JsonNode eventNode, Long runId) {
        try {
            ...
            if (run != null) {
                ...
                statusEventService.record(taskId, runId, "STEP_RECORDED", "running",
                        Map.of("sequence", sequence, "stepType", stepType,
    ...
    private void handleRunCompleted(JsonNode eventNode, Long runId) {
        try {
            ...
            AgentRun run = agentTaskService.getRunById(runId);
            if (run != null) {
                statusEventService.record(run.getTaskId(), runId,
            ...
            if (run != null) {
                statusEventService.record(run.getTaskId(), runId,
                        ...
                        Map.of("errorCode", errorCode,
                                ...
                                "failedTool", failedTool != null ? failedTool : ""));
            }
