...
- ✅ **冒烟测试全绿** — `scripts/smoke-test.ps1` 全部 PASS / 0 FAIL（2026-08-19 实测 48 项；检查项随版本增加，以脚本实时输出为准）
...
0. **恢复 CI 运行** — CI 因 GitHub 账户计费问题暂停（与仓库配置无关），恢复后自动回绿；期间合并前用 `./scripts/run-all-tests.ps1` 本地全量兜底
...
5. **启用数据库备份** — ✅ 新增 `scripts/backup-mysql.sh`（生产容器版：一致性 dump + gzip + 校验 + 按天清理），crontab 示例见脚本头注释；Milvus 快照用 `scripts/backup_milvus.sh`；恢复后务必做一次恢复演练（PRODUCTION_OPS.md 第 8 节）——✅ **2026-09-03 恢复已脚本化并真实演练通过**：`scripts/restore-mysql.sh <dump.sql.gz> --yes --dev`（重建库→灌 dump→关键表核验），开发栈实测 78 表恢复、9 张关键表行数与恢复前逐一吻合；同轮修复 `backup-mysql.sh` 校验环节 SIGPIPE 误判（pipefail 下 `zcat|head|grep -q` 恒返回 141，会把成功备份误删，已改为先截取头部字节再匹配）
...
6. **运行完整冒烟测试**
   ...
   ```
   预期：`==== 结果: N PASS / 0 FAIL ====`（N 随检查项增加而变化，关键是无 FAIL）
7. **运行性能基线**
   ```powershell
...
7a. **分析扩展包剩余人工项**（可选，演示用）— Superset 首启初始化管理员 + 搭建 5 张看板（bigdata/superset/README.md）；标准档加开 ClickHouse（--profile analytics-full）；Grafana 验证 analytics 告警组触发
...
**生成时间**：2026-08-19  
**当前状态**：✅ 所有自动化任务已完成（DeepSeek 已配置、冒烟全绿）  
**剩余**：👆 P0（上线安全）按需执行
