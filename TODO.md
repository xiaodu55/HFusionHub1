...
0. **【紧急】恢复 CI 运行** — GitHub Actions 因「recent account payments have failed or your spending limit needs to be increased」全部 job 3 秒即失败（2026-08-29 确认，main 分支 CI/E2E 连续红）。**2026-08-29 已复查：仓库侧 workflow 配置（触发器/job 定义/static-checks）全部正常，失败纯属账户计费**。需账户所有者到 GitHub Settings → Billing & plans 处理账单或提高 Spending limit（免费账户可将 Actions 用量降到 0 以内，或等待下月额度重置）。在恢复前，PR 门禁（含 eval-offline 阻断）实际失效，合并需本地跑全量测试兜底：**`.\scripts\run-all-tests.ps1`（一键串起 static-checks → Java mvn test → Python pytest → 前端 build+vitest，2026-08-30 新增）**，或手动逐项：`python scripts/static-checks.py` → Java `mvn test` → Python `pytest -q tests` → 前端 `npm run build && npx vitest run`
1. **修改默认密码** — ✅ 2026-08-30 复查：本地 `docker/.env` 与 `deploy/.env` 的全部密码/令牌键已是强随机值（非占位符/弱默认），`scripts/rotate-secrets.sh --check` 可随时复核。如需正式轮换用 `scripts/rotate-secrets.sh` 生成新值并按脚本提示同步两侧 .env，修改后 `docker compose down && docker compose up -d`
