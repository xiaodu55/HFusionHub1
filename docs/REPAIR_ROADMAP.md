...
> 每条均以修复上述锚点为目标，沿用项目既有风格（Flyway V79+、pytest/vitest/JUnit 补测试、不走 R15 冻结的路线）。
...
1. **全量冒烟**：`scripts/smoke-test.ps1`（31+ API + Python 端点 + 前端 + 文档上传→解析→向量化→检索核心链路），预期 0 FAIL（检查项数量随版本增加，以脚本实时输出为准）。
...
-  三大模块测试基线确认（Java 572 / Python 1258 / 前端 49）
...
| S3/M3 | ✅（完整行锁，2026-08-30） | deny/approve/expire 三处 run 状态迁移改为 `AgentRunMapper.transitionRunStatusGuarded` 条件 UPDATE（行级原子判定），resume 失败收敛改 `transitionRunStatusFromAnyActive`（M3 防覆盖真实终态）；收口至 `AgentRunLifecycleService`，仅迁移成功方结算用量；`AgentApprovalRaceGuardTest` 6 例回归 |
