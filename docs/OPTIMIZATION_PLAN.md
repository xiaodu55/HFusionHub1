...
| 编号 | 问题 | 证据 | 修法 | 验收 |
|---|---|---|---|---|
| R16-1 ✅ | **runtime 基线为 ACL 前口径**：`refusal_correctness=0.625`（50/80）冻结于 2026-09-06T04:21Z，早于 ACL 收口（第 35/36 批）；permission 复测已 30/30（原 0/30），基线未随之重冻结 | `evaluation/baseline/runtime_baseline.json`、CHANGELOG 第 35 批「permission 拒答正确率 0/30 → 30/30」 | 真机栈拉起（Docker/Ollama/Java/Python + kb 101 重播种）后全量 220 用例重跑并 `--update-baseline` 重冻结（2026-09-09T03:06Z）：拒答 0.625→**0.9625** ✅、P95 21.7s→16.4s、token/任务 1582→774；检索/引用指标 0.85→0.55 下移——permission 30 例按 ACL 语义拒答不再贡献召回（口径变化）+ 残余疑模型侧行为漂移（reranker 对照实验无差异、检索路径无本轮改动），归因挂 eval-nightly 监测，详见 `docs/CI_GATES.md` 口径注记 | baseline JSON 重冻结 + CI_GATES 注记；**执行中发现并修复 2 项环境级缺陷（R16-20/21）** |
