...
| 编号 | 问题 | 证据 | 修法 | 验收 |
|---|---|---|---|---|
| R16-1 ✅ | **runtime 基线为 ACL 前口径**：`refusal_correctness=0.625`（50/80）冻结于 2026-09-06T04:21Z，早于 ACL 收口（第 35/36 批）；permission 复测已 30/30（原 0/30），基线未随之重冻结 | `evaluation/baseline/runtime_baseline.json`、CHANGELOG 第 35 批「permission 拒答正确率 0/30 → 30/30」 | 真机栈拉起（Docker/Ollama/Java/Python + kb 101 重播种）后全量 220 用例重跑并 `--update-baseline` 重冻结（2026-09-09T03:06Z）：拒答 0.625→**0.9625** ✅、P95 21.7s→16.4s、token/任务 1582→774；检索/引用指标 0.85→0.55 下移——permission 30 例按 ACL 语义拒答不再贡献召回（口径变化）+ 残余疑模型侧行为漂移（reranker 对照实验无差异、检索路径无本轮改动）。**残余差异已闭环归因**：`scripts/eval_retrieval_live.py` 纯检索直测 OVERALL 0.830（normal 0.986/cross_document 0.983/permission 老口径 1.0）证明检索栈健康，差异在答案层丢 sources（模型侧漂移），详见 `docs/CI_GATES.md` 口径注记 | baseline JSON 重冻结 + CI_GATES 注记 + 归因工具入库；**执行中发现并修复 2 项环境级缺陷（R16-20/21）** |
| R16-20 ✅ | **检索成功被调试快照拖垮**：`_debug_candidates` 对 `content=None` 的结果切片崩溃（`'NoneType' object is not subscriptable`），整次检索以 retrieval_error 告终——真机冒烟实录 | `python-ai/app/core/rag/query_router.py:849` | 调试快照空内容容错 `(result.content or "")[:500]`；同族加固 VectorChannel 构造 `SearchResult(content=r.get("content") or "")` | 冒烟复现→修复后不再复发 |
| R16-21 ✅ | **本机 pymilvus 版本漂移**：已装 2.4.15 vs 锁定 3.0.0（milvus-lite 3.1.0 需 3.x 配对）——milvus-lite 写读路径 content 恒为 null（直测回环证实），同时是 4 个既有 `nullable` 失败测试的根因 | `pip show pymilvus` vs `requirements.txt`；HEAD 上 4 测试失败复现 | `pip install pymilvus==3.0.0` 按锁恢复；写读回环复测通过 | 4 个失败测试全绿；kb 101 重播种后检索内容正常 |
...
| R16-12 ✅ | **IT 迁移收尾**：PromptTestSet（Awaitility 化）/ CostWebhookGate（每类独立库隔离）2 类未迁 Testcontainers；且「无 Docker 整类跳过」机制失效——`@DynamicPropertySource` 在 context 加载期即解析数据源属性，容器未就绪时 context 直接 error（assume 永无执行机会） | `docs/adr/ADR-008`、本机 `mvn test` 实测（5 类 error） | 跳过机制修复（`RequireMySqlCondition` ExecutionCondition）✅；**两类迁移完成（2026-09-09）：8/8 收口**——PromptTestSet worker 线程补生产同款 `runAs(tenantId)` 包装（生产调度器本就正确，测试裸线程未复刻）；CostWebhookGate 间歇失败定位为 DATETIME(0) 秒级取整 × `created_at <= endDate` 上界的秒末竞态（ADR-008 第 2 类问题变体，跨秒界修复）；atomicWrite 子用例复跑全绿（「行锁语义差异」实为 cancel 线程缺租户上下文的测试缺陷）。Docker 可用全量 703 过 0 挂 1 跳 BUILD SUCCESS，含 IT 覆盖率 LINE 47.59% | 8/8 全绿 + 双向验证完成 |
...
| 编号 | 问题 | 证据 | 修法 | 验收 |
|---|---|---|---|---|
| R16-15 ✅（定位完成） | **答案标注率 ~50% 随问题类型波动**（ADR-006 开放点） | `docs/adr/ADR-006` 缺口收敛节 | **已定位（2026-09-09 真机对照实验，见 ADR-006 后记）**：normal 20 例纯检索 1.000 vs 答案层 0.750——5/25% 被 insufficient_evidence 误杀；机制 ① 压缩预设 target_ratio=0.3 过激丢答案句（3 例，消融实验修复）② 模型侧过度自认无证据（2 例，漂移）。修复已实施（两步）：① 压缩率单一来源化 + 默认 0.8（修 3 例）② groundless 判据改写容忍——分数先行统一判据（修 2 例模型侧自认）；真机复跑 **5/5 全部修复**（nq-005/007 带引用正常作答），A4 幻觉防护不受损（14/14 测试绿）| 定位与修复实录落盘 ADR-006 后记 |
...
# 第十七轮优化方案（2026-09-09 规划并实施完成：评测双口径 + 技术债清零 + 前端第二轮 + v1.1.0 发布）
