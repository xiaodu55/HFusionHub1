...
| 编号 | 问题 | 证据 | 修法 | 验收 |
|---|---|---|---|---|
| R16-15 ✅（定位完成） | **答案标注率 ~50% 随问题类型波动**（ADR-006 开放点） | `docs/adr/ADR-006` 缺口收敛节 | **已定位（2026-09-09 真机对照实验，见 ADR-006 后记）**：normal 20 例纯检索 1.000 vs 答案层 0.750——5/25% 被 insufficient_evidence 误杀；机制 ① 压缩预设 target_ratio=0.3 过激丢答案句（3 例，消融实验修复）② 模型侧过度自认无证据（2 例，漂移）。修复已实施：压缩率单一来源化（react 读 RAGConfig）+ 默认 0.8，runtime 基线重冻结（citation_acc 0.743→0.757、refusal 持平）；groundless 判据改写容忍挂后续 | 定位结论与修复实录落盘 ADR-006 后记 |
