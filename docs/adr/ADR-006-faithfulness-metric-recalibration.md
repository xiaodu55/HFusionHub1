...
## 后记（2026-09-09，R16-15 开放点定位完成——真机实验）

ADR 正文遗留的开放问题「答案标注率约 50% 因问题类型波动，待定位
（压缩后块号可见性 or 格式遵循）」已用真机对照实验定位：

**实验**：normal 类 20 例，逐例对比 `/api/rag/eval`（纯检索）与
`/api/chat`（答案层）的文档级来源召回。结果：纯检索 **20/20 全命中
（1.000）**，答案层 0.750——**5/20（25%）被 `insufficient_evidence`
拒答，且全部发生在检索已命中的用例上**。

**两个机制（RAG_COMPRESSION_TARGET_RATIO=1.0 的消融实验证实）**：

1. **压缩过激丢答案句（3/5，可复现修复）**：`from_env` 预设
   `target_ratio=0.3` 过于激进——关键答案句被压缩丢弃后，模型如实回答
   "证据不足"，字面判据命中拒答。关闭压缩后 3 例全部 `completed` 且
   recall=1.0。`_safe_compress` 的过激回退（最小安全比）未能拦住。
2. **模型侧过度自认无证据（2/5，无压缩仍拒答）**：nq-005/007 在无压缩
   时模型仍自认证据不足——DeepSeek 模型行为漂移（与本轮 token 1582→774
   同源），属判据 1 的模型侧误触发。

**修复（同日实施）**：压缩率单一来源化——`react._safe_compress` 改读
`RAGConfig.compression.target_ratio`（原硬编码 0.6），默认提至 **0.8**
（消融实验：0.8 与 1.0 各修复 3/5 误杀，0.8 为更保守的中间点），
`RAG_COMPRESSION_TARGET_RATIO` 可覆盖；runtime 基线随之重冻结
（citation_acc 0.743→0.757、faithfulness 0.564→0.579、refusal 0.9625
持平、token 800 在 R17-2 门禁内）。**方法论教训**：单次 runtime 跑批的
LLM 随机方差显著（P95 同配置两次 16.4s vs 28.9s），单跑对比不可靠——
答案层结论以 nightly 累积观测为准，这正是双口径设计的动机。
**groundless 判据改写容忍（同日追加实施，5/5 收口）**：两判据统一为
"分数先行"——答案存在支持度达阈值的实质句即不判无依据（即使模型同时
输出自认文案）；marker + 无实质支持仍拒。真机复跑 nq-005/007（原判据 1
模型侧 2 例）：均 `completed` 且 recall=1.00，答案带 [1] 引用——
**5/5 误杀全部修复**。幻觉防护不受损：A4 修洞②（字面脱节幻觉）仍由
低支持分支兜住，`tests/test_react_evidence_gate.py` 14/14 全绿。
...
## 增补（缺口收敛 · 2026-09-05）：三个 runtime 缺口的侦察结论与处置

runtime 首测暴露三个缺口，逐项侦察结论与处置（红线：不假实现）：

### 1. 权限类拒答 0/30 → 套件设计矛盾，需主体级 ACL（后续工作）

深入分析发现权限用例与其他用例**共享同一批 section**（交集 10 个）：
`permissions-matrix#hr-access` 既是 cd-028"HR 导出员工数据需要什么审批"
（必须回答）的目标，也是 pt-013"HR 的批量导出权限默认是什么状态"
（必须拒答）的目标；`it-support-runbook#security-incident` 上 pt-007
"处置流程是什么"（拒答）与 cd-025"上报时限"（回答）几乎同义。
基于文档标题、section 或关键词的任何守卫都无法区分这两类查询——
唯一自洽的机制是**主体级 ACL**：权限用例以低权限主体发起，受限内容
对其不可检索（检索层 fail-closed），自然落入 insufficient_evidence。
...
- **路由半边（已修）**：chat 快路径有检索结果即直答，操作类请求
  （IntentType.OPERATION）到不了工具层。现 run()/run_stream() 对
  OPERATION 意图追加工具优先指令，模型正确尝试调工具。
- **工具集半边（如实记录）**：套件期望的 8 个业务工具
  （subscribe_plan/query_order_logistics/apply_annual_leave 等）
  代码库中不存在，也无后端可挂（需业务演示端点）。补齐属独立特性
  （declarative_http_tool 对接业务沙箱端点），不造假实现。

### 3. 证据门校准 → 实测不可无误杀，维持默认关闭

实测融合分分布（kb 101，top-5）：超纲查询 [0×5, 0.35~0.62]，
正常查询 [0.57~1.0]——阈值 0.63 可拦全部超纲但误杀 2/12 正常查询
