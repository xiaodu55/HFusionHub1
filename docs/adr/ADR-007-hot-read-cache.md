...
## 实测（2026-09-05 · 单机全栈 · Redis 本地 · 9 个知识库规模）

成对测量（同连接、每轮 DEL 缓存键后对比，n=5）：
- 未命中均值 **62ms** → 命中均值 **40ms**（-35%）
- 与基线脚本（run-all-benchmarks 知识库列表 Avg=54ms）一致；
  当前数据量下列表 SQL 占比 ~30ms，收益随 KB 数与联表统计量线性增长
- 测量方法论教训：逐请求新建 HTTP 连接的脚本会把 ~600ms 连接/代理开销
  混入数字——延迟测量必须复用连接成对对比
