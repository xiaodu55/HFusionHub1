...
- **Flyway**: current `V1–V81`; new scripts must be **V82+**. Never modify existing migrations. New tables MUST include `tenant_id` (unless in `TENANT_IGNORE_TABLES`).
- **Tests**: Java 665 · Python 1381 · Frontend 49 (unit) + 73 (E2E). `scripts/static-checks.py --test-counts` enforces doc/code sync.
...
- **Schema changes**: create a new Flyway `V82+` script; never touch V1–V81. Always add `tenant_id` to new tables.
