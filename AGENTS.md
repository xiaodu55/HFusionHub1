...
- **Flyway**: current `V1–V78`; new scripts must be **V79+**. Never modify existing migrations. New tables MUST include `tenant_id` (unless in `TENANT_IGNORE_TABLES`).
- **Tests**: Java 572 · Python 1248 · Frontend 49 (unit) + 74 (E2E). `scripts/static-checks.py --test-counts` enforces doc/code sync.
...
- **Schema changes**: create a new Flyway `V79+` script; never touch V1–V78. Always add `tenant_id` to new tables.
...
- Full stack: `.\scripts\smoke-test.ps1` (expect 48 PASS / 0 FAIL)
