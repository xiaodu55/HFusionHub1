...
- **Flyway**: current `V1–V80`; new scripts must be **V81+**. Never modify existing migrations. New tables MUST include `tenant_id` (unless in `TENANT_IGNORE_TABLES`).
- **Tests**: Java 595 · Python 1343 · Frontend 49 (unit) + 74 (E2E). `scripts/static-checks.py --test-counts` enforces doc/code sync.
...
- **Schema changes**: create a new Flyway `V81+` script; never touch V1–V80. Always add `tenant_id` to new tables.
