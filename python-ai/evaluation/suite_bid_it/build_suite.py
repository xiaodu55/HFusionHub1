#!/usr/bin/env python
"""Validate the IT 集成行业方案包 suite definitions and emit frozen
``cases.jsonl`` + ``suite_manifest.json`` (P2-4)。

与 ``evaluation/suite_bid/build_suite.py`` 同构，针对 ``evaluation/kb_bid_it``。
...
KB_ROOT = SUITE_DIR.parent / "kb_bid_it"
