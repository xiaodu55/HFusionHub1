#!/usr/bin/env python
"""Build the frozen synthetic 招投标 KB manifest (B2 垂直化).

与 ``evaluation/kb/build_kb_manifest.py`` 同构：每个 ``## [slug] Title``
章节是一个确定性 chunk，稳定 id 为 ``{doc_id}#{slug}``。输出提交入库，
作为 suite_bid 冻结的 ground truth。

本语料为脱敏虚构招标文件（滨海园区项目），仅用于评测与演示种子，
不包含任何真实企业数据。
"""
