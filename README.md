...
> runtime 评测基线（suite 1.1.0，2026-09-05 首次实测，DeepSeek-v4-flash 单机全栈）：越界检索 0 / 错误率 0 / 引用忠实度（按答案标注引用）0.686 / 拒答正确率 0.625（注入 25/25 + 超纲 25/25 通过；权限类 0/30 为已知 ACL 缺口，见 CHANGELOG 第三十二批）/ runtime 召回 0.653（与离线 0.932 的差距为生产检索栈 vs 合成轨差异）。
